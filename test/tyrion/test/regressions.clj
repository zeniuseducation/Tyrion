...
(deftest linear-regression-test
  (time
    (let [x-one (map float (range 50000))
          m 3.0 c 12.0
          y-one (map #(+ (* m %) c) x-one)
          lm-two (do (info "Timing for two args linear-regresion")
                     (time (linear-regression x-one y-one)))
          lm-one (do (info "Timing for one arg linear-regresion")
                     (->> (mat/matrix [x-one y-one])
                          mat/transpose linear-regression time))
          lm-three-map (do (info "Timing for map linear-regresion")
                           (->> (map #(hash-map :a % :b %2) x-one y-one)
                                (linear-regression :a :b)
                                time))
          lm-three-ds (do (info "Timing for ds linear-regresion")
                          (->> (mat/matrix [x-one y-one])
                               (mat/transpose)
                               (ds/dataset [:a :b])
                               (linear-regression :a :b)
